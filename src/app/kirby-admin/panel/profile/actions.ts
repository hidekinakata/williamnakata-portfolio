...
import type { ProfileModel, ProfileTranslationModel } from "@/prismaClient/models"
...
export async function updateProfile(formData: ProfileModel & {
  translations: ProfileTranslationModel[]
}) {
  try {
    const { id, name, email, github, linkedin, instagram, translations } = formData
      ...
      data: {
        ...
        translations: {
          upsert: translations.map((t) => ({
            where: { profileId_language: { profileId: id, language: t.language } },
